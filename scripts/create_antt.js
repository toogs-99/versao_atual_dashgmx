...
const TABLE_NAME = 'antt';

const client = createDirectus(DIRECTUS_URL)
    .with(staticToken(DIRECTUS_TOKEN))
    .with(rest());

async function createANTTTable() {
    ...
    try {
        ...
        await client.request(createCollection({
            ...
            meta: {
                ...
                note: 'Tabela de informações da ANTT (Registro Nacional de Transportadores Rodoviários de Cargas)'
            }
        }));
        ...
        const fields = [
            { name: 'numero_antt', type: 'string', interface: 'input', display: 'Número da ANTT' },
            { name: 'cnpj_cpf', type: 'string', interface: 'input', display: 'CNPJ/CPF' },
            { name: 'nome', type: 'string', interface: 'input', display: 'Nome' },
