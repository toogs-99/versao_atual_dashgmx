...
async function configureFotosAlias() {
    console.log("Configuring 'dados_fotos' alias on 'fotos' <-> 'cadastro_motorista'...");
    ...
    try {
        ...
        console.log(`Found relation ID: ${fotosRel.id} (collection: ${fotosRel.collection})`);

        await client.request(updateRelation(fotosRel.collection, fotosRel.field, {
            meta: {
                one_field: 'dados_fotos',
                one_collection: 'fotos',
                one_deselect_action: 'delete'
            }
        }));

        console.log("✅ Alias 'dados_fotos' configured successfully!");
